...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^vrtoxin_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $VRTOXIN_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       VRTOXIN_BUILD=
    fi
...
function setpaths()
{
    ...
    targetgcckernelversion=$(get_build_var TARGET_VRTOXIN_ARM)
    ...
    export TARGET_VRTOXIN_ROM=$(get_build_var TARGET_VRTOXIN_ROM)
    export TARGET_NDK_VRTOXIN_ROM=$(get_build_var TARGET_NDK_VRTOXIN_ROM)
    export TARGET_GCC_VERSION_ARM64=$(get_build_var TARGET_GCC_VERSION_ARM64)
    ...
    export OFAST_OPTS=$(get_build_var OFAST_OPTS)
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${VRTOXIN_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    VRTOXIN_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/vrtoxin/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the VRToxin model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch vrtoxin_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the VRToxin github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var VRTOXIN_VERSION)
        ZIPFILE=vrtoxin-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.vrtoxin.device | grep -q "$VRTOXIN_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $VRTOXIN_BUILD, run away!"
    fi
}
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                vrtoxin|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function dopush()
{
    ...
    if (adb shell getprop ro.vrtoxin.device | grep -q "$VRTOXIN_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $VRTOXIN_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/vrtoxin/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
